...
begin
  ...
  _had_existing := found;
  if _had_existing and _existing.status in ('confirmed','waitlisted') then
  ...
  if _had_existing then
    update public.rsvps set status=_new_status, waitlist_position=_waitpos, cancelled_at=null
      where id = _existing.id returning id into _new_rsvp_id;
  else
    insert into public.rsvps (event_id, user_id, status, waitlist_position)
      values (_event_id, _user, _new_status, _waitpos) returning id into _new_rsvp_id;
  end if;

  select t.ticket_code into _code from public.tickets t where t.rsvp_id = _new_rsvp_id;
  if _code is null then
    insert into public.tickets (rsvp_id) values (_new_rsvp_id) returning tickets.ticket_code into _code;
  end if;

  return query select _new_rsvp_id, _new_status, _code, _waitpos;
end; $$;